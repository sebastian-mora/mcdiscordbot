...
(crontab -u ubuntu -l 2>/dev/null; echo "* * * * * /usr/bin/ansible-pull -U git@github.com:sebastian-mora/mcdiscordbot.git --only-if-changed --key-file ~/.ssh/ssh_deploy -i hosts  ansible/${ansible_host_name}.yml  >> ~/ansible-pull-logs") | crontab -u ubuntu -
